...
import bombArtifact from './artifacts/contracts/Bomb.sol/Bomb.json';
function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const bankContract = await new ethers.Contract("0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6", bankArtifact.abi)
  ...
  const getTokenContract = async (symbol, bankContract, provider) => {
    ...
    const abi = symbol === 'Matic' ? maticArtifact.abi : (symbol === 'Bomb' ? bombArtifact.abi : usdtArtifact.abi)
